...
- (id)initWithFrame:(CGRect)frame appSession:(SampleApplicationSession *) app
{
    ...
    if (self) {
        ...
        for (int i = 0; i < NUM_AUGMENTATION_TEXTURES; ++i) {
            ...
            NSLog(@"augmentation texture %@", [NSString stringWithCString:textureFilenames[i] encoding:NSASCIIStringEncoding]);
        }
...
- (void)renderFrameQCAR
{
    ...
    NSMutableSet *previousTrackables = [self.trackables mutableCopy];
    
    self.trackables = [NSMutableSet set];
...
//        }
    ...
    NSMutableSet *removedTrackables = [previousTrackables mutableCopy];
    [removedTrackables minusSet:self.trackables];
    if ([removedTrackables count]) {
        if ([self.delegate respondsToSelector:@selector(backgroundTextureView:removedTrackableWithNames:)]) {
            [self.delegate backgroundTextureView:self removedTrackableWithNames:removedTrackables];
        }
    }
...
//    if ([self.delegate respondsToSelector:@selector(backgroundTextureView:trackables:)]) {
//        [self.delegate backgroundTextureView:self trackables:self.trackables];
//    }
